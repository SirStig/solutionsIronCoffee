...
export const projects: Project[] = [
  {
    ...
    summary:
      'An AI music curator that researches before it answers, reading music press, scene blogs and catalog data, then matches every track against a real streaming catalog so the playlist actually plays. Saves straight to Apple Music or YouTube Music with artwork and ordering intact.',
    story:
...

  {
    ...
    highlights: [
      ...
      'Invite only, with single-use invites that expire',
    ],
    tech: [
      'Expo',
      'React Native',
...

  {
    ...
    summary:
      'An all-in-one fitness social platform with workout tracking, a video feed, a 1.7M-food nutrition database, per-muscle recovery modeling and AI coaching, built end to end as co-founder and lead developer. Shut down and on hold, but it is the project I learned the most from.',
    story:
...

  {
    ...
    highlights: [
      ...
      'Memory backend needs zero setup, so tests run with no infra',
    ],
...

  {
    ...
    story:
      'I run a Jellyfin server. FileBot was the one tool that was actually good at renaming, but it stopped there. It did not touch re-encoding, compression or subtitles, and Jellyfin and Plex are both mediocre at handling that themselves. The tools to do it properly existed, FFmpeg and Whisper-based subtitle generation among them, but nothing tied them together in one place for free.\n\nThe first version was JavaFX, chosen so one codebase could ship to every platform. It worked, but it was buggy and janky, and even though Java is the language I first really got into coding with, I have grown to dislike working in it. So over a long stretch of spare time, I rewrote it from scratch in Python with a PySide6 UI, fixing a long list of bugs along the way.\n\nWhat is there now: hardware-accelerated encoding through NVENC, AMF, Quick Sync and VideoToolbox, subtitle generation that runs fully local via faster-whisper, and metadata-driven renaming pulling from eight sources, with GUI and CLI sharing one core, so a batch job is the same code path as a button click. Version 0.5.0 is the first stable release of that rewrite. It still has a long way to go, and I have a lot more planned for it.',
    year: '2026',
    status: 'live',
    categories: ['open-source'],
    highlights: [
...

  {
    ...
    note: 'MIT licensed. Requires a dev build, not Expo Go.',
  },
